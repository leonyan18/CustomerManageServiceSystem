...
public interface ConversationRepository extends JpaRepository<ConversationEntity, Integer> {
    ...
    @Query("select new dto.ConversationDTO(c)FROM ConversationEntity c where c.staff.username like ?1 or c.staff.name like ?1 order by c.endtime desc")
    List<ConversationDTO> findConversationByKeyword(String keyword,Pageable pageable);

    @Query("select count(c) FROM ConversationEntity c where c.staff.username like ?1 or c.staff.name like ?1 ")
    long countConversationBykeyword(String keyword);
